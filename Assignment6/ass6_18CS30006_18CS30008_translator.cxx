...
void symtable::print() {
	...
	cout << setw(112) << setfill ('_') << "_"<< setfill (' ') << endl;
	...
	for (list <sym>::iterator it = table.begin(); it!=table.end(); it++) {
		cout << left << setw(17) << it->name;
		string stype = print_type(it->type);
		cout << left << setw(22) << stype;
		cout << left << setw(17) << it->category;
		cout << left << setw(19) << it->initial_value;
...
sym* conversion (sym* s, string t) {
	...
	if (s->type->type=="INTEGER" ) {
		if (t=="FLOAT") {
			emit ("EQUAL", temp->name, join("int2float(", s->name, ")"));
			return temp;
		}
		else if (t=="CHAR") {
		...
		if (t=="INTEGER") {
			emit ("EQUAL", temp->name, join("float2int(", s->name, ")"));
			return temp;
		}
		else if (t=="CHAR") {
			emit ("EQUAL", temp->name, join("float2char(", s->name, ")"));
			return temp;
		}
		return s;
	}
		...
		if (t=="FLOAT") {
				emit ("EQUAL", temp->name, join("char2float(", s->name, ")"));
				return temp;
			}
		return s;
	}
...
int size_type (symtype* t){
	if(t->type=="CHAR") return CHAR_SIZE;
...
string print_type (symtype* t){
	...
	else if(t->type=="INTEGER") return "int";
